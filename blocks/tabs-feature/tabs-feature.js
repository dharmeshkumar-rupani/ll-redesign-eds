...
export default async function decorate(block) {
  ...
  // Set up interactivity on the sibling UL list (the visual accordion)
  const section = block.closest('.section');
  if (section) {
    const ul = section.querySelector('.default-content-wrapper:last-child > ul');
    if (ul) {
      ...
      items.forEach((item, i) => {
        item.addEventListener('click', () => {
          ...
          const btn = tablist.querySelectorAll('button')[i];
